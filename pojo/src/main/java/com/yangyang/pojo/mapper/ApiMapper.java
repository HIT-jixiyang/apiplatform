...
public interface ApiMapper {
    ...
    @Select("select a.*,b.price from h2_api a,api_price b where a.api_id=b.api_id and b.price_type=2")
    public List<Map> getAllApiAndApiPriceList();
    @Select("select * from h2_api where sp_id=#{sp_id}")
    ...
    @Select("select * from h2_api ")
    public List<Api> getAllApiList();

    @Delete("delete from h2_api where api_id=#{api_id}")
    ...
    @Select("select * from h2_api where api_id=#{api_id}")
    ...
    @Select("select api_id,api_url,api_enabled ,api_method from h2_api")
    public List<Api> getApiHeartList();
    //扫描完心跳，更新statement
    @Update("update h2_api set api_enabled=#{api_enabled} where api_id=#{api_id}")
    public boolean updatestatement(@Param("api_id") String api_id,@Param("api_enabled") Integer api_enabled);
    @Select("SELECT * FROM h2_api WHERE api_category_id=#{api_category_id} ORDER BY api_cost_algorithm_score desc LIMIT 0,#{size};")
    public List<Api> getLowestCostApiList(@Param(value = "api_category_id")String api_category_id,@Param(value = "size") Integer size);
    @Select("SELECT * FROM h2_api WHERE api_category_id=#{api_category_id} ORDER BY api_stable_algorithm_score desc LIMIT 0,#{size};")
    public List<Api> getMostStableApiList(@Param(value = "api_category_id")String api_category_id,@Param(value = "size") Integer size);
    @Select("SELECT * FROM h2_api WHERE api_category_id=#{api_category_id} ORDER BY api_time_algorithm_score desc LIMIT 0,#{size};")
    public List<Api> getMostFastApiList(@Param(value = "api_category_id")String api_category_id,@Param(value = "size") Integer size);
    @Select("SELECT avg(api_average_response_time) FROM  h2_api WHERE api_category_id=#{api_category_id}")
    public Float getAverageResponseTimeByCategoryID(@Param(value = "api_category_id") String category_id);
//@Select("")
    @Select("SELECT AVG(b.price/b.content ) FROM h2_api a,api_price b WHERE a.api_id=b.api_id and b.price_type=\"2\" AND a.api_category_id=#{api_category_id};")
    public Float getAverageCostByCategoryID(@Param(value = "api_category_id") String api_category_id);
@Select("SELECT AVG(api_success_response_ratio) FROM  h2_api WHERE api_category_id=#{api_category_id}")

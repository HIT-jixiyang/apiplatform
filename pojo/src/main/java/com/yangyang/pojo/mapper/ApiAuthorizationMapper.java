...
public interface ApiAuthorizationMapper {
    @Insert("insert into h2_api_app(api_category_id,app_id,create_time,enabled) values(#{api_category_id},#{app_id},#{create_date})")
    public int addAuthorization(ApiAuthorization apiAuthorization);
    @Select("select * from h2_api_app where api_category_id=#{api_category_id} and app_id=#{app_id}")
...
*/
    @Delete("delete from h2_api_app where api_category_id=#{api_category_id} and app_id=#{app_id}")
    ...
    @Delete("delete from h2_api_app where app_id=#{app_id}")
    public int deleteApiAuthorizationByAppIDAndApiID(@Param(value = "app_id") String app_id);
    @Delete("delete from h2_api_app where api_category_id=#{api_category_id}")

...
public interface BillItemMapper {
    //插入账单条目
    @Insert("insert into h2_bill_item(bill_item_id,api_id,app_id) values(#{bill_item_id},#{api_id},#{app_id})")
    public int addBillItem(BillItem bill_item);

    //根据appID与apiid查询账单列表
    @Select("select * from h2_bill_item where app_id=#{app_id} and api_id=#{api_id}")
